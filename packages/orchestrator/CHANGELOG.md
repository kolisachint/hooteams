...
- `createNodeHarnessFactory` accepts `rules` (an array of `{ path, content }` context files) that are injected into every role's system prompt after hoocode's discovered project context — the seam the server uses for `.hooteams/rules/**`. `buildRoleSystemPrompt` accepts the matching `extraContextFiles` input.
