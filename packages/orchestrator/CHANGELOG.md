...
- `RoleConfig` gains optional `appendSystemPrompt` (appendix after the role prompt), `promptGuidelines` (extra guideline bullets), and `skillPaths` (extra skill directories beyond hoocode's defaults).
- New dependency `@kolisachint/hoocode-agent`; `@kolisachint/hoocode-agent-core` and `@kolisachint/hoocode-ai` bumped to `^0.4.66` across the workspace to keep a single resolved version. Note: hoocode's `buildSystemPrompt` export is not in the latest published release yet, so until it ships `buildRoleSystemPrompt` returns the role prompt unchanged (pre-integration behavior) and the rich prompt activates automatically once the export lands.
